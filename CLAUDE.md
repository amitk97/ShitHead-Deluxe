...
- 📤 SHARE (`#matchSummaryShare` → `shareMatchResult`): `drawShareCard(matchShareData())` paints a 1080×1350 PNG (logo, placing, name + mode, rating/Diamond pills, finishing order ≤4, six stats, game link, date) and passes it to `navigator.share({ files })` (WhatsApp, X, Instagram, Photos…); where file sharing isn't supported it downloads `shithead-result.png`. Keep the worst case (4 players + both pills) inside the card.
