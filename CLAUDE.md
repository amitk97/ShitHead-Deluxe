...
- Quick Start (`QUICK_START_MODULE` / `TUTORIAL_MODULE_QUICK_START`): 8 steps, one short sentence each (a test caps steps at 8 and text at 18 words). The lobby Tutorial button starts it directly the first time (`launchTutorialModule(id, { fromHub: false })`, ends on the lobby with "You're ready!"); once done, the button opens the hub, where Quick Start is the highlighted first row. Step 5 makes the player tap the glowing ⓘ to open Card Powers and step 6 the glowing ▦ to open the Play Matrix (`tapCheck: '#cardRefBtn'` / `'#matrixRefBtn'`, completed via `tutorialRefButtonTap` in the buttons' click handlers because they stop propagation); each panel closes when the step moves on and in `endTutorial`, and the note step spotlights the open panel alone. Steps 7–8 empty the Hand and Deck (`scene: { hand: [], emptyDeck: true }`) to show the Face-Up cards, then the Face-Down backs (`faceUp: []`), with the table label lit. It is NOT part of `getAllTutorialModuleIds` (the hub lessons). Challenges (Getting Started): `QUICK_START_CHALLENGE` (Quick Starter, 50) for Quick Start, and `TUTORIAL_COMPLETION_CHALLENGE` (Tutorial Graduate, 200) for Quick Start + every hub lesson; both go through `syncTutorialChallenges` (at load and on module completion; `accountTutorialLessonsDone` mirrors `users/{uid}/tutorialCompleted`, which still means every hub lesson). Keep new first-time teaching here short; detail belongs in the hub lessons.
...
- The play-direction badge (`#gameDirectionBadge`) is a circle the size of the Card Powers button, in the left-hand column above the Play Matrix button (same size), level with the middle of the Deck (`positionDirectionBadge`, end of `render` and on resize). Its arrow is a drawn SVG (`directionArrowSvg`), not the ↻/↺ characters, which fonts place off-centre; it is centred to within 0.1px. Its colour shows the direction: clockwise gold, anticlockwise cyan (`data-direction="cw"/"ccw"` set in `updateDirectionBadge`, colours in the `#gameDirectionBadge[data-direction]` CSS; a test checks they differ). The hand's rows spread into spare height (at least 55% of each back-row card shows), and `fitHandToTable` (end of `render`) shrinks the rows/cards if the player's area still runs past the table bottom (the play button row is revealed after the hand is sized). The hand's `maxHeight` is the cards' height plus its own padding.
- Vs Bots remembers the last bot count and difficulty (`readBotPrefs` / `applyBotPrefs`: localStorage `shithead_bot_count` / `shithead_difficulty`, plus `botCount` / `botDifficulty` in `users/{uid}/settings` for signed-in accounts). A locked difficulty is never restored. Signed in, the lobby instead defaults to the highest unlocked difficulty (`highestUnlockedDifficulty`, picked by `selectHighestUnlockedDifficulty` in `refreshDifficultyLocks` once the account's unlocks load, `difficultyLocksKnownFor`; lobby only, never mid-match); the remembered one is for guests. Bot count and difficulty share one grid (`.bot-setup-grid`) so the count row is centred on the taller difficulty buttons (a test checks).
...
- Offline Vs Bots: `sw.js` precaches the page, icons, `audio/*.mp3`, the Firebase compat scripts and confetti on install (`OFFLINE_ASSETS`, cache `shithead-assets-v1`) and keeps art/fonts as they're used (`isCachedAsset`, stale-while-revalidate). A new file the game needs offline goes in `OFFLINE_ASSETS`; if the Firebase SDK version in index.html changes, change the URLs there too. Nothing that talks to the database or sign-in is cached. Offline, Online Room / Ranked show `OFFLINE_MODE_MSG` instead of opening (`isOffline()`), and the `offline`/`online` events banner. Tested end-to-end with Playwright (needs `PW_EXPERIMENTAL_SERVICE_WORKER_NETWORK_EVENTS=1` so `context.route` sees the worker's requests).
...
- Share icon, top-left of the summary card opposite the ✕ (`#matchSummaryShare.ms-share-icon`: iOS box-and-arrow on Apple devices, three connected dots elsewhere; `SHARE_ICON_APPLE` / `SHARE_ICON_OTHER`) → `shareMatchResult`: `drawShareCard(matchShareData())` paints a 1080×1350 PNG (logo, placing, name + mode, rating/Diamond pills, finishing order ≤4, six stats, game link, date) and passes it to `navigator.share({ files })` (WhatsApp, X, Instagram, Photos…); where file sharing isn't supported it downloads `shithead-result.png`. Keep the worst case (4 players + both pills) inside the card.
...
- Match Stats keep players who left once their result was in (match FINISHED, or they had finished): the room listener hands the old/new lists to `rememberDepartedPlayers`, `buildMatchStats` reads `matchStatsPlayers()` and marks them "(left)"; cleared in `hideMatchEndUI` and on a room change. Every phone also saves everyone's final numbers to `rooms/{code}/finalStats` ({matchId, players}, `saveFinalMatchStats` from `showMatchEndUI`, a transaction that only adds missing players), which brings them back after a reload/rejoin (same matchId, FINISHED). Match History entries carry every player's `st` = [played, picked, burnt, jokers, turns] + `left` (`everyoneTable`; older entries without `st` keep the old layout).
...
- Friends list and Leaderboard are live while open: `renderFriendsList` keeps one `publicProfiles/{uid}` `.on('value')` per friend (`friendProfileWatch`, repaint debounced 150ms) and `openLeaderboardPanel` keeps `leaderboard` `.on('value')` (`leaderboardWatchRef`); both detach when their page is hidden (MutationObserver) or the Friends tab changes. `syncPublicProfile` writes the picture on its own first and into the leaderboard entry in the same `set`, and runs on every equip (unequip-all too).
